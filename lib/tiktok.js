...
return {status: "success", author: "iwan", result : {title:data.data.data.title,sd:`${mainUrl}${data.data.data.play}`,hd:`${mainUrl}${data.data.data.hdplay}`,musik:`${mainUrl}${data.data.data.music}`} }
 }else{
 return {status: "error", author: "iwan", message: "Kesalahan Fetching Data"}
}
}catch(e){
return {status: "error", author: "iwan", message: "Kesalahan Api Panel"}
}
}
